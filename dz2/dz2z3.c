...
void single_iteration_MPI(float *result, float *temp, float *power, int row, int col, float Cap_1, float Rx_1, float Ry_1, float Rz_1, float step) {
    ...
    if (rank == MASTER){
        for (int i = 1; i<size; i++){
            int startt = i * chunk;
            int endd = startt + chunk < row ? startt + chunk : row;
            
            MPI_Isend(result + startt*col, col*(endd-startt), MPI_FLOAT, i, tag_resultsend, MPI_COMM_WORLD, &request);
            
            if ((endd + 1)>row) endd = endd - 1;

            MPI_Isend(temp + (startt-1)*col, col*(endd + 2 - startt), MPI_FLOAT, i, tag_temp, MPI_COMM_WORLD, &request);
            
        }
    }else{
        
        MPI_Recv(result + start*col, col*(end-start), MPI_FLOAT, MASTER, tag_resultsend, MPI_COMM_WORLD, &status);

        int endd = end;

        if ((end + 1)>row) endd = endd - 1;

        MPI_Recv(temp + (start-1)*col, col*(endd + 2 - start), MPI_FLOAT, MASTER, tag_temp, MPI_COMM_WORLD, &status);
    ...
    if (rank == MASTER){
        for (int i = 1; i<size; i++){
            int stigao=0;
            MPI_Recv(&stigao, 1, MPI_INT, MPI_ANY_SOURCE, tag_end, MPI_COMM_WORLD, &status);

            int startt = stigao * chunk;
            int endd = startt + chunk < row ? startt + chunk : row;

            MPI_Recv(result + startt * col, (endd - startt)*col, MPI_FLOAT, stigao, tag_resultrecive, MPI_COMM_WORLD, &status);
        }
    }else{

        MPI_Send(&rank, 1, MPI_INT, MASTER, tag_end, MPI_COMM_WORLD);

        MPI_Isend(result + start * col, (end - start) * col, MPI_FLOAT, MASTER, tag_resultrecive, MPI_COMM_WORLD, &request);
...
void compute_tran_temp_MPI(float *result, int num_iterations, float *temp, float *power, int row, int col) {
    ...
    if (rank == MASTER){
        ...
        for (int i=1; i<size; i++ ){
            ...
            MPI_Isend(power + startt*col, col*(endd-startt), MPI_FLOAT, i, tag_power, MPI_COMM_WORLD, &request);
        }

    } else{

        int start = chunk * rank;
        int end = start + chunk < row ? start + chunk : row;

        MPI_Recv(power + start*col, col*(end-start), MPI_FLOAT, MASTER, tag_power, MPI_COMM_WORLD, &status);
