...
void single_iteration_omp(float *result, float *temp, float *power, int row, int col, float Cap_1, float Rx_1, float Ry_1, float Rz_1, float step) {


#pragma omp parallel for default(none) \
            shared(result) \
            firstprivate(temp, power, row, col, Cap_1, Rx_1, Ry_1, Rz_1, step)\
            schedule(static)
    for (int r = 0; r < row; r++) {
        for (int c = 0; c < col; c++) {
            float delta;
...
void compute_tran_temp_omp(float *result, int num_iterations, float *temp, float *power, int row, int col) {
    int i = 0;
    ...
    float *t = temp;
    for (int i = 0; i < num_iterations; i++) {
        single_iteration_omp(r, t, power, row, col, Cap_1, Rx_1, Ry_1, Rz_1, step);
        float *tmp = t;
        t = r;
        r = tmp;
    }
...
int main(int argc, char **argv) {
    ...
    float *temp, *tempomp, *power, *poweromp, *result, *resultomp;
    char *tfile, *pfile, *ofile, *ofileomp;
    ...
    tempomp = (float *)calloc(grid_rows * grid_cols, sizeof(float));
    power = (float *)calloc(grid_rows * grid_cols, sizeof(float));
    poweromp = (float *)calloc(grid_rows * grid_cols, sizeof(float));
    result = (float *)calloc(grid_rows * grid_cols, sizeof(float));
    resultomp = (float *)calloc(grid_rows * grid_cols, sizeof(float));
    if (!temp || !power) fatal("unable to allocate memory");
    if (!tempomp || !poweromp) fatal("unable to allocate memory");
    ...
    ofile = argv[7];
    const char* omp = "omp";
    asprintf(&ofileomp, "%s%s", argv[7], omp);
   
    read_input(temp, grid_rows, grid_cols, tfile);
    read_input(tempomp, grid_rows, grid_cols, tfile);
    read_input(power, grid_rows, grid_cols, pfile);
    read_input(poweromp, grid_rows, grid_cols, pfile);
    ...
    compute_tran_temp_omp(resultomp, sim_time, tempomp, poweromp, grid_rows, grid_cols);
    ...
    for (int i = 0; i < grid_rows; i++) {
        for (int j = 0; j < grid_cols; j++) {
            ...
            if ((provera==1) && (result[i * grid_cols + j]!=resultomp[i * grid_cols + j])){
                provera=0;
                printf("Test FAILED\n");
                break;
            }
    ...
    writeoutput((1 & sim_time) ? resultomp : tempomp, grid_rows, grid_cols, ofileomp);
